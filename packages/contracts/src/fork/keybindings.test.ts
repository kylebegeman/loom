...
const upstreamCommands = STATIC_KEYBINDING_COMMANDS.filter(
  (command: string) => !forkCommands.includes(command),
);
...
describe("fork keybinding commands", () => {
  ...
  it("never reuse an upstream command", () => {
    expect(upstreamCommands.filter((command) => command.startsWith("loom."))).toEqual([]);
  });
});
