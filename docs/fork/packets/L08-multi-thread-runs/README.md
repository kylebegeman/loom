...
or models in one step, each in its own thread and (by default) its own worktree, grouped as
a run. **Delegation**: an agent can start another thread through exactly two MCP tools,
`loom_multi_thread_runs_start_thread` and `loom_multi_thread_runs_get_thread`, including on a
different provider (a Claude thread hands a task to Codex and later reads the result). The
children are ordinary threads the user can open, steer and stop. A **Runs** panel lists the
...
- In:
  - Compare dialog: prompt, 2 to 6 members (provider instance and model each), workspace
    (separate worktrees or project root), base branch, runtime and interaction mode.
  ...
  - Two MCP tools on T3's existing MCP server, gated by a Loom setting that is off by
    ...
    never above the calling thread's.
...
- Out:
  ...
  - Automatic merging, judging or ranking of compare results.
...
About 2,000 to 2,600 lines including tests: thread starter, run service, MCP toolkit and
reactor about 1,000; contracts 200; web panel, dialog, settings and palette about 1,100.
