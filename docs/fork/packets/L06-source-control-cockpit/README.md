...
- In:
  ...
  - **Checks**: GitHub check runs and commit statuses for the branch head, with or without a
    ...
    agent to fix" puts a prompt with the failing step into the composer.
  - **Conflicts**: the operation in progress, unmerged files with conflict kind and marker
    count, lockfile and generated-file hints, "Open file", "Ask the agent to resolve", and
    copyable continue and abort commands.
...
- Out:
  ...
  - Continue or abort buttons for merges and rebases (commands are shown, not run), restack,
    virtual branches.
...
conflicts, preflight, stash, varlock) ~900, contracts ~250, web panel and views ~1,000, graph
layout ~150, tests ~400. Each view can land as its own commit.
