...
2. Open Source control from the launcher (letter G). Lane shows "Rebase in progress: 1
   ...
   opens it; "Ask the agent to resolve" fills the composer; the commands copy.
3. Resolve by hand, `git add f.txt && git rebase --continue` in the terminal; the banner
   clears after the status stream updates.
4. Make a dirty change to `f.txt`, open Safe switch, pick the other branch: verdict
   "conflict". "Stash and switch" switches; the old branch's lane shows the stash; Pop
   restores it.
