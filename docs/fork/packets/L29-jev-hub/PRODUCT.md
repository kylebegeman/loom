...
authoritative for question shapes and states. "Agents" says whether an MCP tool can reach
the feature (registry `agentTool`); where it cannot, the feature has only "Use Jev" and no
"Let agents use this".
...
| L14    | `chat-conveniences.auto-preset`      | "Auto" preset: a model and effort for the message being typed, from the preset's choices; suggested, never switched silently.                                 | Choice `model` over the Auto preset's choices (labels as keys, the user's descriptions as criteria); Choice `effort`: light, standard, deep (mapped in code to the choice's effort range) | No     | The current selection.                                                           |
...
- **L13 clipboard secret detection:** deciding whether clipboard contents are a secret by
  sending them to a third party defeats the purpose. Local pattern checks in L13's
  `clipboardFilter.ts` cover it without the clipboard leaving the machine.
