...
| `apps/server/src/fork/provider-sign-in/codexTools.test.ts`            | `parseCredentialStore` for unset, `"file"`, `"keyring"`, `"auto"`, a key inside a table (ignored), comments; MCP and skill normalizers drop malformed entries; import items get stable ids.                                                                                                                                                                                                                                                          |
| `apps/server/src/fork/provider-sign-in/ProviderSignInService.test.ts` | `subscribe` switches to the new manager after a registry change and yields `supported: false` for a disabled instance; `signOut` stops sessions for the instance (fake `ProviderService` and `ProviderSessionDirectory`) before calling the manager.                                                                                                                                                                                                 |
| `apps/web/src/fork/provider-sign-in/signIn.logic.test.ts`             | Default method by client location; phase labels; `findSameAccountLabel` (case-insensitive email, same driver only, ignores signed-out).                                                                                                                                                                                                                                                                                                              |
...
6. Credential store: set `cli_auth_credentials_store = "keyring"` in the scratch shared
   `config.toml`; the notice appears for the shadow instance; **Use file storage** rewrites it.
