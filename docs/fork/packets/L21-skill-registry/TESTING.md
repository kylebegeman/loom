...
| `apps/server/src/fork/skill-registry/targets.test.ts`        | Kyle's layout (four account folders symlinked to `~/.claude/skills`): one shared target seen by all four, no account rows, shared is the default; an account with its own folder gets an "Only" row; no shared readers: first account is the default; no Claude instance: `agentsUser` is the default; `agentsUser` lists Codex and Cursor instances; a missing `~/.agents/skills` resolves with `realDir: null`. |
...
| `apps/server/src/fork/skill-registry/installs.test.ts`       | A missing target folder is created on the first install; copy writes the marker and record; refuses an existing folder; skips symlinks pointing outside the clone; update replaces and moves the old copy to trash; update refuses a modified install without `force`; remove moves to trash and refuses non-Loom skills. Uses `SqlitePersistenceMemory`.                                                         |
| `apps/server/src/fork/skill-registry/lab.test.ts`            | `scaffold` writes the frontmatter (both booleans), creates a missing target folder, refuses an existing folder and records `source_url` `"lab"`; `writeSkill` fails with a conflict on a stale `expectedHash`; `readSkill` and `writeSkill` refuse a path outside an editable entry; a lab-created skill can be removed to trash.                                                                                 |
...
| `apps/web/src/fork/skill-registry/inventory.logic.test.ts`   | Filters (provider, scope, state, search), badge derivation, shared-folder note text, grouping of same-name source entries, preselection per target family (a missing subpath is skipped).                                                                                                                                                                                                                         |
...
5. Run "Install skills from git" from the palette: Skills opens on Sources with the URL field
   ...
   both scratch accounts; "Check for updates" says up to date; remove it; it is in the trash
   folder. Install one skill into the scratch project's `.agents/skills`, which does not
   exist yet; the folder is created.
