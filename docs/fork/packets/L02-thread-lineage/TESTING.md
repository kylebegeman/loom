...
1. Open a thread with at least ten messages on Claude. Hover an assistant message in the
   middle, click "Fork from here". The dialog shows the copied count and has New worktree
   selected. Choose Conversation, type "Summarize what we decided so far." Fork. The app
   opens the child: the history is visible up to that message, the first message shows a
   "Forked conversation" chip, and the reply reflects the earlier conversation. On a thread long enough to exceed about 90,000 characters, the dialog shows "Earlier
   messages trimmed", the child's chip reads "Forked conversation (earlier messages
   trimmed)" and a toast confirms it.
