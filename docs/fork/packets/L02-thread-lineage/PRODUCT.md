...
- **Choose where the fork runs.** Keep the source's workspace, use the project root, or
  create a new worktree from the source's branch. Pick another provider or model, for
  example fork a Claude thread into Codex.
...
Settings: none in v1. The fork dialog remembers the last context and workspace choices per
browser (`loom:thread-lineage:fork-defaults:v1`).
...
- Workspace options: "Same as source", "Project root", "New worktree".
...
- Truncation: when the transcript exceeds the budget, the dialog says "The oldest N messages
  are left out of the model's context" before forking.
...
- Should a fork default to "Same as source" workspace (shares uncommitted files with the
  parent) or to "New worktree" (isolated, but starts from the last commit, not the parent's
  uncommitted changes)? The packet defaults to "Same as source" for sidecars and to "New
  worktree" for forks when the source uses a worktree.
- Should the transcript budget (about 90,000 characters, oldest messages dropped first) be
  configurable?
