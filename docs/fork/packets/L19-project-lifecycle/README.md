...
the clone location (default `~/Developer/active`) as a Loom project, adopt an existing clone
...
offers to push what is missing, and then moves the folder to the Trash on the machine that
holds it. Nothing is ever deleted permanently.

## Scope

- In:
  - A `/loom/repositories` page with an environment picker, search and filters (All,
    Cloned, Not cloned, Parked).
  ...
  - Park: safety assessment, "Push all branches and tags", acknowledgement of kept ignored
    files, optional archiving of the project's threads, move to the Trash (checkout plus
    clean linked worktrees), and a parked record.
  ...
  - Loom settings section: clone location per environment, extra "safe to lose" and "keep"
    patterns.
...
- Out:
  ...
  - A sidebar entry or a project context-menu item (both would be packet seams; see
...
- [`ext-keybindings`](../EXTENSION-POINTS.md#9-keybindings-ext-keybindings) (command `loom.project-lifecycle.open`), which requires [`ext-web-root`](../EXTENSION-POINTS.md#5-web-root-ext-web-root)
  (a shortcut host component).
...
No edits to upstream source lines. Details in [SEAMS.md](./SEAMS.md).
...
contracts ~200, web page, dialogs and settings ~800, tests ~400.
